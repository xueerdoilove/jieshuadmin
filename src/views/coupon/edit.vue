  ...
  <div>
    <el-form ref="form" :rules="rules"  :model="newcoupon" label-width="120px">
      ...
      <el-form-item label="优惠券总数量" prop="couponCnt">
        <el-input class="riqi" type="number" v-model="newcoupon.couponCnt"></el-input> 已领取数量{{newcoupon.applyCnt}}
      </el-form-item>

      

      <el-form-item label="作用">
        <el-select class="riqi" v-model="newcoupon.couponType" :disabled="true" placeholder="请选择优惠券类型">
...
import { putcoupon } from '@/api/coupon'
import qs from 'qs'
export default {
  name: "newcoupon",
  data() {
  ...
  created(){
    ...
    var a = JSON.parse(localStorage.getItem('editcoupon'))
    if(a.ruleType ==1){
      a.timeRange = [a.startDate,a.endDate]
      a.startTime = a.startDate
      a.endTime = a.endDate
    }else{
      a.startDate = ''
      a.endDate = ''
    ...
    onSubmit(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          putcoupon(this.newcoupon.id,formData).then(res =>{
            this.$emit('hideedit');
          })
        } else {
