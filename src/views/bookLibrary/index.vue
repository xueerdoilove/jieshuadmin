<template>
  <div class="container">
    <el-row class="tianjian_tou">
      <el-col :span="4">
        <el-select class="riqi" v-model="searchtype" @change="changepage(1)">
          <el-option
            v-for="searchdtype in searchtypelist"
            :key="searchdtype.id"
            :label="searchdtype.name"
            :value="searchdtype.id"
          ></el-option>
        </el-select>
      </el-col>
      <el-col :span="15" :offset="1">
        <el-form label-width="80px" v-show="searchtype==0">
          <el-form-item label="状态:">
            <el-radio-group v-model="state" @change="zhuangtai">
              <el-radio :label="2">上架中</el-radio>
              <el-radio :label="1">待上架</el-radio>
              <el-radio :label="0">已下架</el-radio>
            </el-radio-group>
          </el-form-item>
        </el-form>

        <el-input placeholder="请输入图书名称/作者/isbn" v-model="searchvalue" v-show="searchtype==1"  class="input-with-select">
          <el-select v-model="state" slot="prepend" placeholder="请选择状态" style="width:100px;">
            <el-option label="已上架" :value="2"></el-option>
            <el-option label="待上架" :value="1"></el-option>
            <el-option label="已下架" :value="0"></el-option>
          </el-select>
          <el-button slot="append" @click="searchbookbykeyword" icon="el-icon-search">搜索</el-button>
        </el-input>

        <el-autocomplete
          class="input-with-select"
          v-show="searchtype==2"
          v-model="storeitem"
          :fetch-suggestions="querySearchAsync"
          placeholder="请输入门店名"
          @select="searchbookbystore"
          value-key="name"
        ></el-autocomplete>
      </el-col>
      <el-col :span="4" style="text-align:right">
        <el-button type="primary">+新增</el-button>
      </el-col>
    </el-row>

    <el-row class="caozuo_tou">
      <el-col :span="4" :offset="0">
        <el-checkbox v-model="checkedall">全选</el-checkbox>
      </el-col>
      <el-col :span="20" style="text-align:right">
        <el-button type="primary">编辑</el-button>
      </el-col>
    </el-row>

    <el-row class="book_div">
      <el-col :span="12" v-for="book in booklist" v-bind:key="book.id">
        <bookbigitem :bookData="book" />
      </el-col>
    </el-row>

    <el-row class="tiaojian_item" v-show="totalItems>=pageSize">
      <el-col :span="20" :offset="2" style="text-align:center;">
        <el-pagination
          background
          :total="totalItems"
          :page-size="pageSize"
          :current-page="page"
          @current-change="changepage"
          layout="prev, pager, next"
        ></el-pagination>
      </el-col>
    </el-row>
    <el-row class="tiaojian_item" v-show="totalItems==0">
      <el-col :span="20" :offset="2" style="text-align:center;">没有书籍</el-col>
    </el-row>
  </div>
</template>

<script>
import { getbookList , bookcipsearch , bookcipbystore} from "@/api/book";
import Bookbigitem from "./bookbigitem";
import { getstoreList } from "@/api/store";

export default {
  named: "书库信息",
  data() {
    return {
      searchtype: 0,
      searchtypelist: [
        { id: 0, name: "状态" },
        { id: 1, name: "书名/作者/isbn" },
        { id: 2, name: "门店" }
      ],



      checkedall: false,
      booklist: [],
      state: 2, //下架(0),未上架(1),上架(2)）
      count: -1, // -1 无,
      page: 1,
      pageSize: 10,

      searchvalue:'',// 搜索书的关键字

      storeid:0,
      storelist:[],
      storeitem:'',

      totalItems: 0
    };
  },
  components: {
    Bookbigitem
  },
  mounted() {
    this.getbooklist();
  },
  methods: {
    // 根据关键字查询书籍
    searchbookbykeyword(){
      if(this.searchvalue==''){
        this.booklist=[];
        this.totalItems = 0
        return
      }
      bookcipsearch({
        state:this.state,
        name:this.searchvalue,
        page:this.page,
        pageSize:this.pageSize
      }).then(res =>{
        this.booklist = res.items
        this.totalItems = res.totalItems
      })
    },
    // 异步去请求门店数据
    querySearchAsync(queryString, cb) {
      if(this.storelist.length==0){
        getstoreList({
          page: 1,
          pageSize: 100,
          state: 2,
        })
          .then(res => {
            this.storelist = res.items;
            var restaurants = this.storelist;
            var results = queryString ? restaurants.filter(this.createStateFilter(queryString)) : restaurants;
            cb(results);
          })
      }else{
        var restaurants = this.storelist;
        var results = queryString ? restaurants.filter(this.createStateFilter(queryString)) : restaurants;
        cb(results);
      }
    },
    createStateFilter(queryString) {
      return (state) => {
        return (state.name.toLowerCase().indexOf(queryString.toLowerCase()) === 0);
      };
    },
    searchbookbystore(item){
      console.log(item,'adfa')
      this.page = 1;
      this.storeid = item.id
      this.getbookbystore()
    },
    getbookbystore(){
      bookcipbystore({
        bookStoreId:this.storeid,
        page:this.page,
        pageSize:this.pageSize,
      }).then(res =>{
        this.booklist = res.items
        this.totalItems = res.totalItems
      })
    },
    getbooklist() {
      getbookList({
        state: this.state,
        page: this.page,
        pageSize: this.pageSize
      })
        .then(res => {
          this.booklist = res.items;
          this.totalItems = res.totalItems;
        })
        .catch(error => {});
    },
    zhuangtai(e) {
      this.page = 1;
      this.getbooklist();
    },
    changepage(e) {
      this.page = e;
      if(this.searchtype==0){
        this.getbooklist();
      }else if(this.searchtype==2){
        this.getbookbystore();
      }else{
        this.searchbookbykeyword()
      }
    }
  }
};
</script>

<style lang="scss" scoped>
.container {
  margin: 30px;
}
.tianjian_tou {
  border-bottom: 1px solid #aaa;
}
.caozuo_tou {
  padding: 15px 0 15px 30px;
}
.book_div {
  margin-top: 10px;
}
.tiaojian_item {
  margin-top: 20px;
}
.input-with-select{
  margin-bottom: 20px;
}
</style>
